...
standard_conc_file <- paste0(wd, exp, "/Transition-Lists/quant-molecules-standard-concentrations.csv")
...
# Function to handle extracting parts of the sample name
  extract_sample_parts <- function(sample_name) {
      ...
      if ("Plasma" %in% parts) {
        if ("quant" %in% parts) {
          Sample.Type <- 'Plasma'
          donor <- as.integer(parts[4])
          sampling_day <- as.integer(parts[5])
          timepoint <- as.integer(parts[6])
          Standard <- NA
          replicate <- NA
          
        } else if (any(grepl("^S\\d+$", parts))) {
          Sample.Type <- 'Plasma'
          donor <- 'pool'
          sampling_day <- NA
          timepoint <- NA
          Standard <- as.character(parts[3])
          replicate <- as.character(parts[4])
    ...
    return(c(Sample.Type, donor, sampling_day, timepoint, Standard, replicate))
  }
  ...
  metadata_df$Standard <- as.character(extracted_parts[, 5])
  metadata_df$replicate <- as.character(extracted_parts[, 6])
...
standard_conc <- read.csv(standard_conc_file)
standard_conc <- t(standard_conc)
colnames(standard_conc) <- standard_conc[1,]

# Merge more data into metadata df
metadata_df <- merge(metadata_df, all_storage_data, by = "Sample.ID", all.x = TRUE)
metadata_df <- merge(metadata_df, delivery_data, by.x = 'Donor.Sampling.Day', by.y = 'Sample.(Proband.to.Food)', all.x = TRUE)
...
rownames(standard_conc) <- as.character(rownames(standard_conc))
# Function to get the matching standard row or fallback to S3
get_standard_values <- function(sample_name) {
  match <- rownames(standard_conc)[sapply(rownames(standard_conc), function(x) grepl(x, sample_name))]
  if (length(match) > 0) {
    return(standard_conc[match[1], , drop = FALSE])
  } else {
    return(standard_conc["S3", , drop = FALSE])
  }
}

# Apply to all rows in metadata_df
standard_values_list <- lapply(metadata_df$Sample, get_standard_values)

# Combine into a single dataframe
standard_values_df <- do.call(rbind, standard_values_list)

# Combine with metadata_df
metadata_df <- cbind(metadata_df, standard_values_df)
