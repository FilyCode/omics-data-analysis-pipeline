...
wd = "//prot-data/MS Storage 3/Exploris480/2025/"
setwd(wd)

# exp = "250321_CVPlasmen_Philipp"
# exp = "250324_CVPlasmen2_Philipp"
# exp = "250327_CVPlasmen3_Philipp"
# exp = "250331_Philipp_FiS_CVs"
# exp = "250401_Philipp_Plasmen_CVs"
 exp = "250324_CVPlasmen2_Philipp/hpos"
...
# Remove potential wrong samples
# full_prep_data <- full_prep_data %>% filter(!error)
# full_prep_all_data <- full_prep_all_data %>% filter(!error)
...
# Calculate CV for targeted molecule
  ...
  # to get CV of technical replicates
    technical_cv_data <- full_prep_data %>%
      group_by(Molecule.Name, replicate) %>%
      summarise(
        Mean_Area = mean(Area, na.rm = TRUE),
        SD_Area = sd(Area, na.rm = TRUE),
    ...
    # Plot: Histogram of CVs
    plot <- ggplot(technical_cv_data, aes(x = CV)) +
                geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.7) +
    ...
    ggsave(paste0(resultsdir, "CV-histogram-targeted-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    
    # Plot: CV vs. Mean Area (Scatter)
    plot <- ggplot(technical_cv_data, aes(x = min_mean_Area, y = CV, label = Molecule.Name)) +
                geom_point(color = "blue", alpha = 0.6) +
                geom_text_repel(
                  size = 3, 
                  box.padding = 0.3,      # Smaller padding around text
    ...
    ggsave(paste0(resultsdir, "CV-vs-Mean-scatter-plot-targeted-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    
    # Plot: CV vs. RT (Scatter)
    plot <- ggplot(technical_cv_data, aes(x = rt, y = CV, label = Molecule.Name)) +
      geom_point(color = "blue", alpha = 0.6) +
      geom_text_repel(
        size = 3, 
        box.padding = 0.3,      # Smaller padding around text
    ...
    ggsave(paste0(resultsdir, "CV-vs-RT-scatter-plot-targeted-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
...
# Calculate CV for each molecule
  ...
  # to get CV of technical replicates
    technical_all_cv_data <- full_prep_all_data %>%
      group_by(id, replicate) %>%
      summarise(
        Mean_Area = mean(Area, na.rm = TRUE),
        SD_Area = sd(Area, na.rm = TRUE),
    ...
    # Plot: Histogram of CVs
    plot <- ggplot(technical_all_cv_data, aes(x = CV)) +
      geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.7) +
      labs(title = "Distribution of CV of all Molecules\nof technical replicates",
           x = "CV (%)", y = "Count") +
      theme_minimal() +
    ...
    ggsave(paste0(resultsdir, "CV-histogram-all-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    
    # Plot: CV vs. Mean Area (Scatter)
    plot <- ggplot(technical_all_cv_data, aes(x = min_mean_Area, y = CV, label = Annotation)) +
      geom_point(color = "blue", alpha = 0.6) +
      scale_x_log10() +  # Log scale for better visualization
      labs(title = "CV vs. Mean Area of all Molecules\nof technical replicates",
           x = "Mean Area (log scale)", y = "CV (%)") +
    ...
    ggsave(paste0(resultsdir, "CV-vs-Mean-scatter-plot-all-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    # Plot: CV vs. RT (Scatter)
    plot <- ggplot(technical_all_cv_data, aes(x = rt, y = CV, label = Annotation)) +
      geom_point(color = "blue", alpha = 0.6) +
      labs(title = "CV vs. RT of all Molecules\nof technical replicates",
           x = "RT (min)", y = "CV (%)") +
      theme_minimal() +
    ...
    ggsave(paste0(resultsdir, "CV-vs-RT-scatter-plot-all-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
...
# normalize with Caffeine-D9
    ...
    # Remove potential wrong samples
    # full_prep_data_norm <- full_prep_data_norm %>% filter(!error)
    # full_prep_all_data_norm <- full_prep_all_data_norm %>% filter(!error)
    ...
    # to get CV of technical replicates
    technical_cv_data_norm <- full_prep_data_norm %>%
      group_by(Molecule.Name, replicate) %>%
      summarise(
        Mean_Area = mean(Area, na.rm = TRUE),
        SD_Area = sd(Area, na.rm = TRUE),
    ...
    technical_cv_data_norm$CV_raw <- technical_cv_data$CV
    technical_cv_data_norm$diff <- technical_cv_data_norm$CV_raw - technical_cv_data_norm$CV
    technical_cv_data_norm$color <- ifelse(technical_cv_data_norm$diff < 0, "red", "blue")
    
    # Plot: Histogram of CVs
    plot <- ggplot(technical_cv_data_norm, aes(x = CV)) +
      geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.7) +
      labs(title = "Distribution of CV of normalized targeted Molecules\nof technical replicates",
           x = "CV (%)", y = "Count") +
      theme_minimal() +
    ...
    ggsave(paste0(resultsdir, "CV-histogram-targeted-normalized-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    
    # Plot: CV vs. Mean Area (Scatter)
    plot <- ggplot(technical_cv_data_norm, aes(x = min_mean_Area, y = CV, label = Molecule.Name, color = color)) +
      geom_point(alpha = 0.6) +
      geom_text_repel(
        size = 3, 
        box.padding = 0.3,      # Smaller padding around text
    ...
    ggsave(paste0(resultsdir, "CV-vs-Mean-scatter-plot-targeted-normalized-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    
    # Plot: CV vs. RT (Scatter)
    plot <- ggplot(technical_cv_data_norm, aes(x = rt, y = CV, label = Molecule.Name, color = color)) +
      geom_point(alpha = 0.6) +
      geom_text_repel(
        size = 3, 
        box.padding = 0.3,      # Smaller padding around text
      ...
      labs(title = "CV vs. RT of targeted Molecules\nof normalized technical replicates",
           x = "RT (min)", y = "CV (%)") +
      scale_color_identity() +  # Use the colors from the 'color' column
      theme_minimal() +
      theme(
    ...
    ggsave(paste0(resultsdir, "CV-vs-RT-scatter-plot-targeted-normalized-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    ...
    plot <- ggplot(all_cv_data_norm, aes(x = rt, y = CV, label = Annotation, color = color)) +
      geom_point(alpha = 0.6) +
      labs(title = "CV vs. RT of all normalized Molecules",
           x = "RT (min)", y = "CV (%)") +
      scale_color_identity() +  # Use the colors from the 'color' column
      theme_minimal() +
      theme(
    ...
    # to get CV of technical replicates
    technical_all_cv_data_norm <- full_prep_all_data_norm %>%
      group_by(id, replicate) %>%
      summarise(
        Mean_Area = mean(Area, na.rm = TRUE),
        SD_Area = sd(Area, na.rm = TRUE),
    ...
    technical_all_cv_data_norm$CV_raw <- technical_all_cv_data$CV
    technical_all_cv_data_norm$diff <- technical_all_cv_data_norm$CV_raw - technical_all_cv_data_norm$CV
    technical_all_cv_data_norm$color <- ifelse(technical_all_cv_data_norm$diff < 0, "red", "blue")
    
    # Plot: Histogram of CVs
    plot <- ggplot(technical_all_cv_data_norm, aes(x = CV)) +
      geom_histogram(binwidth = 1, fill = "steelblue", color = "black", alpha = 0.7) +
      labs(title = "Distribution of CV of all normalized Molecules\nof technical replicates",
           x = "CV (%)", y = "Count") +
      theme_minimal() +
    ...
    ggsave(paste0(resultsdir, "CV-histogram-all-normalized-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    
    # Plot: CV vs. Mean Area (Scatter)
    plot <- ggplot(technical_all_cv_data_norm, aes(x = min_mean_Area, y = CV, label = Annotation, color = color)) +
      geom_point(alpha = 0.6) +
      scale_x_log10() +  # Log scale for better visualization
      labs(title = "CV vs. Mean Area of all normalized Molecules\nof technical replicates",
           x = "Mean Area (log scale)", y = "CV (%)") +
    ...
    ggsave(paste0(resultsdir, "CV-vs-Mean-scatter-plot-all-normalized-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
    
    # Plot: CV vs. RT (Scatter)
    plot <- ggplot(technical_all_cv_data_norm, aes(x = rt, y = CV, label = Annotation, color = color)) +
      geom_point(alpha = 0.6) +
      labs(title = "CV vs. RT of all normalized Molecules\nof technical replicates",
           x = "RT (min)", y = "CV (%)") +
      scale_color_identity() +  # Use the colors from the 'color' column
    ...
    ggsave(paste0(resultsdir, "CV-vs-RT-scatter-plot-all-normalized-technical.png"), plot = plot, width = 8, height = 6, dpi = 300)
